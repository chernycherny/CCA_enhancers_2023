...
# bap1 binding sites: diff_TBTS5_WT_vs_EV_BAP1_c3.0_cond1.bed
# bap1mutant binding sites: diff_TBTS5_MUT_vs_EV_BAP1_c3.0_cond1.bed

# format file for Homer
cd /home/gmsv1178/CCA/CCA_ChipSeq/processed/diffbind_macs2bdgdiff
for i in diff_TBTS5_WT_vs_EV_BAP1_c3.0_cond1.bed diff_TBTS5_MUT_vs_EV_BAP1_c3.0_cond1.bed
do
  basefilename=$(basename $i .bed)
  echo $basefilename
  grep -E "^chr[0-9]+\s" $i > $basefilename.filt1.bed
  sort -k1,1V -k2,2n -k3,3n $basefilename.filt1.bed > $basefilename.filt2.bed
  cat $basefilename.filt2.bed | awk '{print $1 "\t" $2 "\t" $3 "\t" $1":"$2"-"$3 "\t" $5 "\t" "+"}' > $basefilename.formatHomer.bed
  rm $basefilename.filt1.bed
  rm $basefilename.filt2.bed
done
...
# switch to conda python 3.7 environment, for Homer / deseq2
source activate py37
cd /home/gmsv1178/CCA/CCA_ChipSeq/processed
mkdir homer
for i in diffbind_macs2bdgdiff/*.formatHomer.bed
do
  samplename=$(basename $i)
  echo $samplename
  qsub -pe smp 4 findMotifsGenome.pl $i ~/genome/GRCh37.p13.genome.chr_only.fa homer/${samplename}.homer -size given -p 4
done
