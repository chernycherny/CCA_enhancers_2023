...
geneset_genes = unique(c(msigdb_H_list$HALLMARK_INTERFERON_GAMMA_RESPONSE, msigdb_H_list$HALLMARK_ALLOGRAFT_REJECTION, msigdb_H_list$HALLMARK_INTERFERON_ALPHA_RESPONSE, msigdb_H_list$HALLMARK_COMPLEMENT, msigdb_H_list$HALLMARK_IL2_STAT5_SIGNALING, msigdb_H_list$HALLMARK_IL6_JAK_STAT3_SIGNALING, msigdb_H_list$HALLMARK_INFLAMMATORY_RESPONSE))
...
zz_mix_fluke = val_enhancers_multicomp$Tumor_EGroupMixed_vs_Tumor_EGroupFluke$gain_exp_enhgain[, c("gene", "enhancer", "Fold.x.1", "p.value.x.1", "Fold.x.2", "p.value.x.2", "Fold.y.1", "p.value.y.1", "expr_fold.x.1", "expr_p.x.1", "expr_fold.x.2", "expr_p.x.2")]

zz_mix_C4 = val_enhancers_multicomp$Tumor_EGroupMixed_vs_Tumor_EGroupC4$gain_exp_enhgain[, c("gene", "enhancer", "Fold.x.1", "p.value.x.1", "Fold.x.2", "p.value.x.2", "Fold.y.1", "p.value.y.1", "expr_fold.x.1", "expr_p.x.1", "expr_fold.x.2", "expr_p.x.2")]

mix_gain = merge(zz_mix_fluke, zz_mix_C4, by=c('enhancer','gene'), suffixes=c('.Fluke', '.C4'), sort=F)

# for each gene, select one enhancer
mix_gain_split = split(mix_gain, mix_gain$gene)
set.seed(123)
zz = lapply(mix_gain_split, function(x){return (x[sample(1:nrow(x), size=1), ])}) # select random enhancer 
mix_gain = do.call(rbind, zz)
set.seed(777)
mix_gain = mix_gain[sample(nrow(mix_gain)),] # randomize order because val_enhancers_multicomp puts genes with no cell-line expression at bottom
mix_gain$enhancergene = paste0(mix_gain$enhancer, '|', mix_gain$gene)
mix_enhancergene = mix_gain[, c("enhancergene", "enhancer", "gene")]



######### get enhancers & genes NOT GAINED in mix vs normal, mix vs fluke, mix vs C4 (enhancer & expression)

### mix vs normal
contrast = 1
# get diffbind results for all enhancer
my_report = data.frame(dba.report(K27ac_dba_deseq_contrasts3, contrast=contrast, method=DBA_DESEQ2, th=1, bCounts = T, bNormalized = T), check.names = F)
colnames(my_report)[6:ncol(my_report)] = colnames(mcols(dba.report(K27ac_dba_deseq_contrasts3, contrast=contrast, method=DBA_DESEQ2, th=0.05, bCounts = T, bNormalized = T)))
...
diffexpr = diffexpr_list_contrasts3[[contrast]]$diffexpr
...
mix_norm_nogain = my_report[my_report$`p-value` > .3 & my_report$expr_p > .3,]
...
my_report$enhancergene = paste0(my_report$enhancer, "|", my_report$gene)
# get nogain ehhancers
mix_fluke_nogain = my_report[my_report$`p-value` > .3 & my_report$expr_p > .3,]
...
my_report$enhancergene = paste0(my_report$enhancer, "|", my_report$gene)
# get nogain ehhancers
mix_C4_nogain = my_report[my_report$`p-value` > .3 & my_report$expr_p > .3,]

mix_nogain_enhancergene = intersect(intersect(mix_norm_nogain$enhancergene, mix_fluke_nogain$enhancergene), mix_C4_nogain$enhancergene)
mix_nogain_enhancergene = data.frame(enhancergene=mix_nogain_enhancergene, stringsAsFactors = F)
mix_nogain_enhancergene = cbind(mix_nogain_enhancergene, do.call(rbind, strsplit(mix_nogain_enhancergene$enhancergene, '|', fixed=T)), stringsAsFactors=F)
colnames(mix_nogain_enhancergene)[2:3] = c("enhancer", "gene")
mix_nogain_enhancergene = mix_nogain_enhancergene[!duplicated(mix_nogain_enhancergene$enhancer),] # remove duplicate enhancers

# for each gene, select one enhancer
mix_nogain_enhancergene_split = split(mix_nogain_enhancergene, mix_nogain_enhancergene$gene)
set.seed(123)
zz = lapply(mix_nogain_enhancergene_split, function(x){return (x[sample(1:nrow(x), size=1), ])}) # select random enhancer 
mix_nogain_enhancergene = do.call(rbind, zz)
# select 30 to plot
set.seed(999)
mix_nogain_enhancergene = mix_nogain_enhancergene[sample(1:nrow(mix_nogain_enhancergene), size=30),]
...
plot_enhancers = rbind(mix_enhancergene, mix_nogain_enhancergene)
plot_enhancers$gain = F
plot_enhancers[plot_enhancers$enhancergene %in% mix_enhancergene$enhancergene, "gain"] = T
